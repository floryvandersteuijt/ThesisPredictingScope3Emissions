...
# fill inconvenient NA's with 1000 
...
## Figures ##
######################################
Figure_scope <- read_excel("~/Documents/Thesis/Robustness/Robustness checks2.xlsx", 
                           sheet = "Figure")

Figure_scope %>%  
  ggplot(aes(x = Year, y = Companies, 
             colour = Scope, group = Scope)) +
  geom_line() +
...
Final_dataset_log_winso %>% select(c(2:14)) %>% 
  cor() %>% corrplot()
